...
test("the startup hook opens the call tab without taking the focus", async () => {
  const calls: Array<{ method: string; params: Record<string, unknown> }> = [];
  const result = await startOrFocusCall({
    pluginRoot: "/plugins/herdr-call",
    focus: false,
    herdr: {
      async request(method, params) {
        calls.push({ method, params });
        if (method === "pane.list") {
          return { panes: [{ pane_id: "w1:p1", focused: true, cwd: "/home/me" }] };
        }
        return { plugin_pane: { pane: { pane_id: "w1:p2" } } };
      },
    },
  });
...
test("the startup hook hands the focus back after probing a call pane that is already open", async () => {
  const calls: Array<{ method: string; params: Record<string, unknown> }> = [];
  const result = await startOrFocusCall({
    pluginRoot: "/plugins/herdr-call",
    focus: false,
    herdr: {
      async request(method, params) {
        ...
        if (method === "pane.list") {
          return {
            panes: [
              { pane_id: "w1:p1", focused: true, cwd: "/home/me" },
              { pane_id: "w1:p2", label: "Herdr Call", cwd: "/plugins/herdr-call" },
            ],
          };
        }
        return { type: "plugin_pane_focused" };
  ...
  assert.deepEqual(result, { status: "focused", paneId: "w1:p2" });
  assert.deepEqual(calls, [
    { method: "pane.list", params: {} },
    { method: "plugin.pane.focus", params: { pane_id: "w1:p2" } },
    { method: "pane.focus", params: { pane_id: "w1:p1" } },
  ]);
});

test("a restored shell pane does not stop the startup hook from opening the call", async () => {
  const calls: Array<{ method: string; params: Record<string, unknown> }> = [];
  const result = await startOrFocusCall({
    pluginRoot: "/plugins/herdr-call",
    focus: false,
    herdr: {
      async request(method, params) {
        ...
        if (method === "plugin.pane.focus") {
          throw new HerdrRequestError("plugin_pane_not_found", "plugin pane not found");
        }
        return { plugin_pane: { pane: { pane_id: "w1:p3" } } };
      },
    },
  });

  assert.deepEqual(result, { status: "opened", paneId: "w1:p3" });
